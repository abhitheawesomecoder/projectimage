...
class AjaxController extends BaseController
{
    ...
    public function UploadPremiumIcon() {
       ...
       // $tags_icons = TagsIcons::get();
        ...
        $html = '<span style="float: left; padding-right: 10px; padding-bottom: 5px; color: #000;"><b>Wählen</b> Sie ein Icon aus:</span>
                <a id="color-blue" class="color-blue active"></a>
                <a id="color-black" class="color-black"></a>
                <a id="color-green" class="color-green"></a>
                <a id="color-yellow" class="color-yellow"></a>';

        foreach($tags_icons as $icon) {

             $html .= '<a id="color-'.$icon->id.'" class="color-'.$icon->id.'" style="margin-left: 5px; margin-top: 5px; display: inline-block;"><img width="30" height="30" src="'.$icon->image.'"></a>';
        ...
        try {
            ...
            foreach ($image->tags()->get() as $img) {
                ...
                foreach ($image_tags as $tag) {
                    $product = Product::where('id' , '=' , $tag->product_id )->delete();
                }
            }
